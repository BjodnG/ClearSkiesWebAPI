...
var SkjemaComp = (function () {
    function SkjemaComp(_http, fb) {
        ...
        this.skjema = fb.group({
            Epost: ['', forms_1.Validators.pattern("^[\\w!#$%&'*+\\-/=?\\^_`{|}~]+(\\.[\\w!#$%&'*+\\-/=?\\^_`{|}~]+)*@((([\\-\\w]+\\.)+[a-zA-Z]{2,4})|(([0-9]{1,3}\\.){3}[0-9]{1,3}))$")],
            Sporsmaal: ['', forms_1.Validators.required]
        });
        this.nyttSporsmaal = new NyeSporsmaal_1.NyeSporsmaal(0, '', '');
    }
    //Metoder:
    SkjemaComp.prototype.postNyttSporsmaal = function () {
        ...
        this._http.post("api/StdSpm/", this.nyttSporsmaal)
            .subscribe(function (error) { return console.log('Valideringsfeil på server: \n\r' + error); }, function () { return alert('Feil å server. Prøv igjen senere.'); });
